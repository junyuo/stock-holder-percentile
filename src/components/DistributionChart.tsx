      ...
      <div className="chart-wrap">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} layout="vertical" margin={{ top: 8, right: 12, left: 8, bottom: 4 }}>
            <CartesianGrid strokeDasharray="3 6" horizontal={false} stroke="#dce7e5" />
            <XAxis
              type="number"
            ...
            <YAxis
              ...
              fontSize={10}
            />
            <Tooltip content={<ChartTooltip metric={metric} />} cursor={{ fill: '#eef6f4' }} />
            <Bar dataKey="plottedValue" radius={[0, 6, 6, 0]}>
              {data.map((item) => (
                <Cell fill={item.level === activeLevel ? '#13a87e' : '#8baaa4'} key={item.level} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
